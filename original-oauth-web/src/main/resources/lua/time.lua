...
local function getTime(timeZone)
    ...
    --local data = redis.call("GET", SEQUENCE_TIME);
    local result = {};
    --if ("false" == tostring(data)) then
    ...
    --    data = string.format("%s,%s,%s,%s,%s,%s,", time, day, yearEst, month, dom, hour);
    --    redis.call("SETEX", SEQUENCE_TIME, expire, data);
    --else
