...
public class KafkaMessageQueueConsumerFactory implements MessageQueueConsumerFactory {

	private static final Logger LOG = LoggerFactory.getLogger(KafkaMessageQueueProducerFactory.class);

	private final Map<String, MessageQueueConsumerHandler> HANDLER_POOL = new ConcurrentSkipListMap<String, MessageQueueConsumerHandler>();
	...
	public void RegisterHandler(MessageQueueConsumerHandler handler) {
		HANDLER_POOL.put(handler.getQueueName(), handler);
	}

	public void RegisterHandler(List<MessageQueueConsumerHandler> handlers) {
		for (MessageQueueConsumerHandler consumerHandler : handlers) {
	...
	public KafkaMessageQueueConsumerFactory init(Map<String, Object> args) {
		...
		listening = new Thread(new Runnable() {
			public void run() {
				...
				try {
					MessageQueueConsumerHandler messageQueueConsumerHandler;
